                ...
                <div className="d-flex flex-column justify-content-center">
                    <nav className="d-flex justify-content-center m-3 navbar navbar-light">
                        <form className="d-flex" onSubmit={this.submit}>
                            <input className="form-control me-2" value={this.state.search} onChange={(e) => { this.setState({ search: e.target.value }) }} type="search" placeholder="Search" required />
                        ...
                        {this.state.left && this.state.articles.map((element) => {
                            ...
                            return (
                                <div key={element.symbol}>
                                    {(regex.test(element.symbol) || regex.test(element.name)) &&
